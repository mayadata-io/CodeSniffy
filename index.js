...
module.exports = (app) => {
  app.on(
    ...
    async (context) => {
      ...
      const customGeneratedFiles = await PR.getCustomGeneratedFiles(
        context,
        owner,
        repo
      );
      ...
      res.data.forEach(function (item) {
        ...
        if (
          PR.globMatch(item.filename, customGeneratedFiles) ||
          g.isGenerated()
        ) {
      ...
      if (title.includes("litmus-portal")) {
        await AddLabel.addLabel(
          context,
          "area/litmus-portal",
          Config.colors["area/litmus-portal"]
        );
      ...
      // assign GitHub label
      return await AddLabel.addLabel(
        context,
        labelToAdd,
