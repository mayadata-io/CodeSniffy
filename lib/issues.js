...
function issueOpened(app) {
    ...
    app.on(['issues.opened','issues.edited','issues.reopened'], async context => {
        ...
        const kuberaLabelToAdd = 'area/kubera-chaos';
        ...
        // create a comment
        if(body.toLowerCase().includes('kubera-chaos') || body.toLowerCase().includes('kubera chaos')){
            AddLabel.addLabel(context, kuberaLabelToAdd, Config.colors[kuberaLabelToAdd])
        }
    })
}
